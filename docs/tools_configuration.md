...
4claw's tools configuration is located in the `tools` field of `config.json`.
...
By default, 4claw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `FOURCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `FOURCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `FOURCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `FOURCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
