...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'unknown_provider' not supported in 4claw, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in 4claw, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.4claw/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.4claw/workspace")
		}
	})
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.4claw/workspace"},
...
func TestRunDryRun(t *testing.T) {
	...
	picoWs := filepath.Join(fourClawHome, "workspace")
	if _, err := os.Stat(filepath.Join(picoWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(fourClawHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	picoConfig, err := config.LoadConfig(filepath.Join(fourClawHome, "config.json"))
	if err != nil {
		t.Fatalf("loading 4claw config: %v", err)
	}
